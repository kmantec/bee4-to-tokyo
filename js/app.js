...
function renderHome() {
  ...
  app().innerHTML = `
    <div class="animate-fade-in">
      ...
      <div class="px-5 mt-3">
        <button onclick="showScreen('learn')" class="premium-card w-full p-5 rounded-2xl flex items-center gap-4 text-left border-amber-200 bg-amber-50/50 mb-3"><div class="text-3xl">🧠</div><div class="flex-1"><p class="font-display text-xl font-bold text-navy-500">Learn Mode</p><p class="text-sm text-navy-500/60">S.P.E.L.L. techniques and mistake training</p></div></button>
...
async function speakLearnWord(word) {
  try {
    ...
    // ใช้ engine เดียวกับ Listening ที่ทำงานได้แล้ว
    await speak(word, { rate: 0.72 });   // ช้า ชัด
    await new Promise(resolve => setTimeout(resolve, 200));
    await speak(word, { rate: 0.95 });   // ความเร็วปกติ
  } catch (e) {
    console.warn('Learn speech failed:', e);
  }
}
...
function renderLearnMode(){
  const n4=HIGH_VALUE_WORDS.filter(w=>LEARN_CARDS[w][3]===4).length,n5=HIGH_VALUE_WORDS.filter(w=>LEARN_CARDS[w][3]===5).length,n3=HIGH_VALUE_WORDS.filter(w=>LEARN_CARDS[w][3]===3).length;
  app().innerHTML = `<div class="animate-fade-in px-5 pt-8 pb-28"><div class="flex items-center gap-3 mb-5"><button onclick="showScreen('home')" class="text-navy-500/60 hover:text-navy-500 text-2xl">←</button><div><p class="text-xs font-semibold text-navy-500/50 tracking-wider uppercase">Technique training</p><h1 class="font-display text-3xl font-bold text-navy-500">Learn Mode</h1></div></div><div class="hero-gradient diagonal-pattern text-white rounded-3xl p-6 mb-5"><p class="font-display italic text-sm opacity-80">S.P.E.L.L. System</p><p class="font-display text-3xl font-bold leading-tight mt-1">Learn patterns, not just words.</p><p class="text-sm opacity-85 mt-3">Structure · Pronunciation · Error Pattern · Logic · Speed</p></div><div class="grid grid-cols-2 gap-3 mb-5"><button onclick="window.b4t.startLearnDrill('pattern')" class="premium-card rounded-2xl p-4 text-left"><div class="text-3xl mb-2">🧩</div><p class="font-display font-bold text-navy-500">Pattern Drill</p><p class="text-xs text-navy-500/60">suffix, prefix, family</p></button><button onclick="window.b4t.startLearnDrill('error')" class="premium-card rounded-2xl p-4 text-left"><div class="text-3xl mb-2">🎯</div><p class="font-display font-bold text-navy-500">Error Instinct</p><p class="text-xs text-navy-500/60">choose correct spelling</p></button><button onclick="window.b4t.startLearnDrill('brainstorm')" class="premium-card rounded-2xl p-4 text-left"><div class="text-3xl mb-2">💡</div><p class="font-display font-bold text-navy-500">Brainstorm Logic</p><p class="text-xs text-navy-500/60">definition → word</p></button><button onclick="window.b4t.startLearnDrill('mixed')" class="premium-card rounded-2xl p-4 text-left border-amber-200 bg-amber-50/60"><div class="text-3xl mb-2">🏆</div><p class="font-display font-bold text-navy-500">Mixed Challenge</p><p class="text-xs text-navy-500/60">all techniques</p></button><button onclick="window.b4t.startLearnDrill('speed')" class="premium-card rounded-2xl p-4 text-left"><div class="text-3xl mb-2">⚡</div><p class="font-display font-bold text-navy-500">Speed Recall</p><p class="text-xs text-navy-500/60">fast answer training</p></button><button onclick="window.b4t.reviewLearnMistakes()" class="premium-card rounded-2xl p-4 text-left"><div class="text-3xl mb-2">📌</div><p class="font-display font-bold text-navy-500">Mistake Bank</p><p class="text-xs text-navy-500/60">review weak words</p></button></div><div class="premium-card rounded-2xl p-4 mb-5"><p class="font-display font-bold text-navy-500 text-lg">Coverage</p><p class="text-sm text-navy-500/70">${HIGH_VALUE_WORDS.length} high-value words · Bee4 ${n4} · Bee5 ${n5} · Bee3 ${n3}</p><p class="text-xs text-navy-500/50 mt-1">Tap LEARN to open a mini lesson and quick quiz.</p></div><h2 class="font-display text-lg font-bold text-navy-500 mb-3">High-value Patterns</h2><div class="flex gap-2 overflow-x-auto pb-3 mb-3"><button onclick="window.b4t.filterLearnCards('all')" class="btn-secondary px-3 py-2 rounded-full text-xs font-bold">All</button><button onclick="window.b4t.filterLearnCards('S')" class="btn-secondary px-3 py-2 rounded-full text-xs font-bold">Structure</button><button onclick="window.b4t.filterLearnCards('P')" class="btn-secondary px-3 py-2 rounded-full text-xs font-bold">Sound</button><button onclick="window.b4t.filterLearnCards('E')" class="btn-secondary px-3 py-2 rounded-full text-xs font-bold">Error</button><button onclick="window.b4t.filterLearnCards('L')" class="btn-secondary px-3 py-2 rounded-full text-xs font-bold">Logic</button></div><div id="learnCardList" class="space-y-3">${HIGH_VALUE_WORDS.map(renderLearnCard).join('')}</div></div>`;
}
function filterLearnCards(cat){const words=cat==='all'?HIGH_VALUE_WORDS:HIGH_VALUE_WORDS.filter(w=>LEARN_CARDS[w][4]===cat);const el=$('#learnCardList');if(el)el.innerHTML=words.map(renderLearnCard).join('')||`<div class="premium-card rounded-2xl p-6 text-center">No cards.</div>`;}
function renderLearnCard(w){const c=LEARN_CARDS[w]; if(!c) return ''; return `<div class="premium-card rounded-2xl p-4"><div class="flex justify-between gap-3 mb-2"><div><div class="flex items-center gap-2"><p class="font-display font-bold text-xl text-navy-500">${escapeHtml(w)}</p><button onclick="event.stopPropagation(); window.b4t.speakLearnWord('${escapeHtml(w)}')" class="bg-navy-50 hover:bg-navy-100 text-navy-500 rounded-full w-9 h-9 flex items-center justify-center text-base" title="Listen">🔊</button></div><p class="text-[10px] uppercase tracking-wider font-semibold text-navy-500/45">Bee ${c[3]} · ${CAT[c[4]]}</p></div><button onclick="window.b4t.openLearnCard('${escapeHtml(w)}')" class="text-[10px] uppercase tracking-wider font-semibold text-amber-700 bg-amber-100 px-3 py-1.5 rounded-full hover:bg-amber-200">Learn</button></div><p class="text-sm text-navy-500/75"><b>Structure:</b> ${escapeHtml(c[0])}</p><p class="text-sm text-navy-500/75"><b>Sound:</b> <span class="font-mono">${escapeHtml(c[1])}</span></p><p class="mt-2 bg-amber-50 rounded-xl p-3 text-sm text-navy-500"><b>Tip:</b> ${escapeHtml(c[2])}</p></div>`;}
function openLearnCard(word){const c=LEARN_CARDS[word]; if(!c)return; app().innerHTML=`<div class="animate-fade-in min-h-screen flex flex-col px-5 pt-8 pb-6"><div class="flex items-center gap-3 mb-5"><button onclick="showScreen('learn')" class="text-navy-500/60 text-2xl">←</button><div><p class="text-xs tracking-wider uppercase font-semibold text-navy-500/50">Mini lesson · Bee ${c[3]}</p><div class="flex items-center gap-2"><h1 class="font-display text-3xl font-bold text-navy-500">${escapeHtml(word)}</h1><button onclick="window.b4t.speakLearnWord('${escapeHtml(word)}')" class="bg-navy-50 hover:bg-navy-100 text-navy-500 rounded-full w-11 h-11 flex items-center justify-center text-xl" title="Listen">🔊</button></div></div></div><div class="hero-gradient diagonal-pattern text-white rounded-3xl p-6 mb-4"><p class="text-xs uppercase tracking-wider opacity-70 mb-2">${CAT[c[4]]}</p><p class="font-display text-2xl font-bold">${escapeHtml(c[0])}</p><div class="flex items-center gap-3 mt-3"><p class="font-mono text-lg opacity-90">${escapeHtml(c[1])}</p><button onclick="window.b4t.speakLearnWord('${escapeHtml(word)}')" class="bg-white/15 hover:bg-white/25 text-white rounded-full px-3 py-2 text-sm font-bold">🔊 Listen</button></div></div><div class="premium-card rounded-3xl p-5 mb-4 space-y-3"><p><b>Rule:</b> ${escapeHtml(c[2])}</p><p><b>Common trap:</b> <span class="text-red-600">${escapeHtml(c[5])}</span></p><p class="bg-navy-50 rounded-xl p-3"><b>Example:</b> ${escapeHtml(c[8])}</p></div><button onclick="window.b4t.startCardQuiz('${escapeHtml(word)}')" class="btn-primary w-full py-4 rounded-xl font-bold">Quick Quiz</button><button onclick="showScreen('learn')" class="btn-secondary w-full py-4 rounded-xl font-bold mt-3">Back</button></div>`;}
function startCardQuiz(word){const c=LEARN_CARDS[word];if(!c)return;state.session={type:'learn-card',items:[[c[6],c[7],c[2],word]],index:0,correct:0,answers:[]};renderLearnDrill();}
function startLearnDrill(kind){let items;if(kind==='pattern'||kind==='speed')items=PATTERN_DRILLS;else if(kind==='error')items=ERROR_DRILLS;else if(kind==='brainstorm')items=BRAINSTORM_DRILLS;else if(kind==='mixed')items=[...PATTERN_DRILLS.map(x=>['pattern',...x]),...ERROR_DRILLS.map(x=>['error',...x]),...BRAINSTORM_DRILLS.map(x=>['brainstorm',...x])];else items=PATTERN_DRILLS;state.session={type:'learn-'+kind,items:shuffleArray(items).slice(0,kind==='mixed'?10:6),index:0,correct:0,answers:[]};renderLearnDrill();}
function renderLearnDrill(){const s=state.session;let it=s.items[s.index],kind=s.type.replace('learn-','');if(kind==='mixed'){kind=it[0];it=it.slice(1);}if(s.type==='learn-card')kind='card';let body='';if(kind==='pattern'||kind==='speed'||kind==='card'){body=`<p class="text-xs uppercase tracking-wider font-semibold text-navy-500/50 mb-3">${kind==='speed'?'Fast recall':kind==='card'?'Card quiz':'Complete the pattern'}</p><p class="font-display text-3xl font-bold text-navy-500 mb-4">${escapeHtml(it[0])}</p><p class="text-sm text-navy-500/60 mb-4">Hint: ${escapeHtml(it[2])}</p><input id="learnAnswer" class="spell-input w-full px-4 py-4 bg-amber-50/40 border-2 border-amber-200 rounded-xl text-xl text-center" autofocus>`;}else if(kind==='error'){body=`<p class="text-xs uppercase tracking-wider font-semibold text-navy-500/50 mb-3">Choose the correct spelling</p><div class="grid gap-3">${shuffleArray([it[0],it[1]]).map(x=>`<button onclick="window.b4t.submitLearn('${escapeHtml(x)}')" class="btn-secondary py-4 rounded-xl font-mono text-lg font-bold">${escapeHtml(x)}</button>`).join('')}</div>`;}else{body=`<div class="flex items-center gap-2 mb-4"><span class="level-pill level-4">${escapeHtml(it[0])}</span><span class="font-display font-bold text-3xl text-navy-500">${escapeHtml(it[1])}____</span></div><p class="text-lg text-navy-500 mb-3">${escapeHtml(it[2])}</p><p class="text-sm text-navy-500/60 mb-4">Strategy: ${escapeHtml(it[4])}</p><input id="learnAnswer" class="spell-input w-full px-4 py-4 bg-amber-50/40 border-2 border-amber-200 rounded-xl text-xl text-center" autofocus>`;}const title=kind==='pattern'?'Pattern Drill':kind==='error'?'Error Instinct':kind==='brainstorm'?'Brainstorm Logic':kind==='speed'?'Speed Recall':kind==='card'?'Quick Quiz':'Mixed Challenge';app().innerHTML=`<div class="animate-fade-in min-h-screen flex flex-col px-5 pt-8 pb-6"><div class="flex items-center gap-3 mb-6"><button onclick="showScreen('learn')" class="text-navy-500/60 text-2xl">←</button><div><p class="text-xs tracking-wider uppercase font-semibold text-navy-500/50">Learn Mode · ${s.index+1}/${s.items.length}</p><h1 class="font-display text-3xl font-bold text-navy-500">${title}</h1></div></div><div class="premium-card rounded-3xl p-6 mb-4">${body}</div>${kind==='error'?'<button onclick="showScreen(\'learn\')" class="btn-secondary w-full py-4 rounded-xl font-bold mt-auto">Back</button>':'<button onclick="window.b4t.submitLearn()" class="btn-primary w-full py-4 rounded-xl font-bold mt-auto">Check</button>'}</div>`;const inp=$('#learnAnswer');if(inp)inp.addEventListener('keydown',e=>{if(e.key==='Enter')window.b4t.submitLearn();});}
function submitLearn(choice){const s=state.session;let it=s.items[s.index],kind=s.type.replace('learn-','');if(kind==='mixed'){kind=it[0];it=it.slice(1);}if(s.type==='learn-card')kind='card';const guess=normalizeAnswer(choice||($('#learnAnswer')?.value||''));const rawAnswer=(kind==='pattern'||kind==='speed'||kind==='card')?it[1]:kind==='error'?it[1]:it[3];const answer=normalizeAnswer(rawAnswer);const lesson=(kind==='pattern'||kind==='speed'||kind==='card')?it[2]:kind==='error'?it[2]:it[4];const ok=guess===answer;if(ok)s.correct++;else logLearnMistake(rawAnswer);s.answers.push({answer:rawAnswer,guess,ok,lesson});showLearnFeedback(ok,rawAnswer,guess,lesson);}
function showLearnFeedback(ok,answer,guess,lesson){app().innerHTML=`<div class="animate-fade-in min-h-screen flex flex-col px-5 pt-10 pb-6"><div class="text-center mb-6"><div class="text-6xl mb-3">${ok?'✅':'🧠'}</div><p class="font-display text-3xl font-bold text-navy-500">${ok?'Correct!':'Learn this pattern'}</p></div><div class="premium-card rounded-3xl p-6 mb-4"><p class="text-xs uppercase tracking-wider font-semibold text-navy-500/50 mb-2">Answer</p><p class="font-display text-3xl font-bold text-navy-500 mb-2">${escapeHtml(answer)}</p>${!ok?`<p class="text-sm text-red-600 mb-3">Your answer: ${escapeHtml(guess||'(blank)')}</p>`:''}<p class="bg-amber-50 rounded-xl p-4 text-navy-500"><b>Technique:</b> ${escapeHtml(lesson)}</p>${LEARN_CARDS[String(answer).toLowerCase()]?`<div class="mt-4">${renderLearnCard(String(answer).toLowerCase())}</div>`:''}</div><button onclick="window.b4t.nextLearn()" class="btn-primary w-full py-4 rounded-xl font-bold mt-auto">Continue</button></div>`;}
function scoreLabelLearn(x){return x>=0.9?'Excellent pattern control':x>=0.7?'Good — review mistakes':x>=0.5?'Building skill':'Needs focused review'}
function nextLearn(){const s=state.session;s.index++;if(s.index<s.items.length)renderLearnDrill();else app().innerHTML=`<div class="animate-fade-in min-h-screen flex flex-col px-5 pt-10 pb-6"><div class="text-center mb-6"><p class="text-xs tracking-wider uppercase font-semibold text-navy-500/50 mb-1">Learn Mode Result</p><p class="font-display text-7xl font-bold text-navy-500 num-badge">${s.correct}<span class="text-3xl text-navy-500/50">/${s.items.length}</span></p><p class="font-display italic text-lg text-navy-500/70 mt-2">${scoreLabelLearn(s.correct/s.items.length)}</p></div><div class="premium-card rounded-2xl p-4 mb-4"><p class="font-bold text-navy-500 mb-2">Next best step</p><p class="text-sm text-navy-500/70">If score is below 70%, open Mistake Bank and repeat weak words only.</p></div><button onclick="showScreen('learn')" class="btn-primary w-full py-4 rounded-xl font-bold mt-auto">Back to Learn Mode</button></div>`;}
function getLearnMistakes(){try{return JSON.parse(localStorage.getItem('b4t_learn_mistakes')||'[]')}catch{return[]}}
function logLearnMistake(word){const a=getLearnMistakes();a.push({word:String(word).toLowerCase(),date:todayKey()});localStorage.setItem('b4t_learn_mistakes',JSON.stringify(a.slice(-80)));}
function reviewLearnMistakes(){const words=[...new Set(getLearnMistakes().map(x=>x.word))].reverse();app().innerHTML=`<div class="animate-fade-in px-5 pt-8 pb-28"><button onclick="showScreen('learn')" class="mb-4 text-navy-500/60">← Back</button><h1 class="font-display text-3xl font-bold text-navy-500 mb-4">Mistake Bank</h1>${words.length?`<div class="space-y-3 mb-4">${words.map(w=>renderLearnCard(w)||`<div class="premium-card rounded-2xl p-4"><p class="font-display font-bold text-xl text-navy-500">${escapeHtml(w)}</p><p class="text-sm text-navy-500/60">Review spelling and meaning.</p></div>`).join('')}</div><button onclick="window.b4t.startLearnDrill('mixed')" class="btn-primary w-full py-4 rounded-xl font-bold">Train Mixed Challenge</button>`:`<div class="premium-card rounded-2xl p-6 text-center"><p class="text-4xl mb-2">🌟</p><p class="font-display font-bold text-xl text-navy-500">No mistakes yet</p><p class="text-sm text-navy-500/60 mt-1">Do drills first. Mistakes will appear here automatically.</p></div>`}</div>`;}
function learnMiniCard(word){const c=LEARN_CARDS[String(word).toLowerCase()]; if(!c)return ''; return `<div class="mt-2 p-3 bg-amber-50 rounded-xl text-xs text-navy-500/75"><b>🧠 Learn:</b> ${escapeHtml(c[0])}<br><b>Sound:</b> <span class="font-mono">${escapeHtml(c[1])}</span><br><b>Tip:</b> ${escapeHtml(c[2])}</div>`;}
